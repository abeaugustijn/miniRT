...
/*   Updated: 2020/03/03 09:13:48 by abe              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rayres		obj_dist_cylinder(t_object *cy, t_ray ray, t_info *info)
{
	...
	if (float_compare(pow(vec_dotp(ray.direction, cy->orientation), 2),
				vec_dotp(ray.direction, ray.direction) * vec_dotp(cy->orientation, cy->orientation)))
		return (rayres_inf());
	double dist = fabs(vec_dotp(vec_from_to(ray.origin, cy->location), vec_crossp(ray.direction, cy->orientation)))
		/ vec_len(vec_crossp(ray.direction, cy->orientation));
	...
	if (dist > cy->size / 2)
		return (rayres_inf());
	return (rayres_new_dist(cy, vec_new(1,0,0), cy->color, 0));
}
