...
/*   Updated: 2020/02/18 12:54:27 by aaugusti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rayres		obj_dist_sphere(t_object *sp, t_ray ray, t_info *info)
{
	...
	(void)info;
	t = vec_dotp(vec_sub(sp->location, CAM->location), ray.direction);
	if (t < 0)
		return (rayres_inf());
	p = vec_add(CAM->location, vec_multiply(ray.direction, t));
